const STORAGE_KEY = "temptrack_pro_demo_v2_professional";
...
function dashboardView() {
  ...
  return `
    <div class="grid kpi-grid">
      ...
      ${kpi("Weekly Hours", s.totalHours.toFixed(2), "Regular + OT", "gray")}
    </div>
          ...
          ${state.agencies.map(a => `
            <div class="list-item">
              <div>
                ...
                <div class="muted small">${a.agencyNumber} · ${a.plan} Plan</div>
              </div>
...
function kpi(label, value, note, color) {
  return `
    <div class="card kpi-card">
      ...
      <p class="note"><span class="badge ${color === "gray" ? "gray" : color}">${note}</span></p>
    </div>
  `;
}
...
function agenciesView() {
  return `
    <div class="grid two-col">
      ...
      <div class="card">
        <h3>Multi-Tenant Control</h3>
        <p class="muted">Every client, worker, job site, punch, and timesheet is tied to a unique agency number.</p>
  ...
  return `
    <div class="grid two-col">
      ...
      <div class="card">
        ...
        <p class="muted">Clients can approve hours, review attendance, report no-shows, and request additional workers.</p>
      </div>
    </div>
    <div class="card" style="margin-top:14px;">
      <h3>Clients for ${agency().agencyNumber}</h3>
      ${scoped("clients").length ? `<div class="table-wrap"><table><thead><tr><th>Client</th><th>Contact</th><th>Email</th></tr></thead>
      <tbody>${scoped("clients").map(c => `<tr><td>${c.name}</td><td>${c.contact}</td><td>${c.email}</td></tr>`).join("")}</tbody></table></div>` : empty("No clients added yet.")}
    </div>
  `;
}
  ...
  return `
    <div class="grid two-col">
      ...
      <div class="card">
        ...
        <p class="muted small">Use this link to create a QR code for the selected agency and job site.</p>
      </div>
    </div>
    <div class="card" style="margin-top:14px;">
      <h3>Job Sites</h3>
      ${scoped("sites").length ? `<div class="table-wrap"><table><thead><tr><th>Client</th><th>Site Code</th><th>Address</th><th>GPS</th><th>QR Link</th></tr></thead>
      <tbody>${scoped("sites").map(s => `<tr><td>${clientName(s.clientId)}</td><td>${s.siteCode}</td><td>${s.address}</td><td>${s.gpsRequired ? "Required" : "Optional"}</td><td>${qrLink(s)}</td></tr>`).join("")}</tbody></table></div>` : empty("No job sites added yet.")}
    </div>
  `;
}
  ...
  return `
    <div class="grid two-col">
      ...
      <div class="card">
        ...
        <p class="muted small">Worker IDs inherit the agency number so reports stay organized.</p>
      </div>
    </div>
    <div class="card" style="margin-top:14px;">
      <h3>Workers</h3>
      ${scoped("workers").length ? `<div class="table-wrap"><table><thead><tr><th>Worker #</th><th>Name</th><th>Phone</th><th>Status</th><th>Assigned Site</th></tr></thead>
      ...
      }).join("")}</tbody></table></div>` : empty("No workers added yet.")}
    </div>
  `;
}
...
function reportsView() {
  ...
  return `
    <div class="grid kpi-grid">
      ${kpi("Total Hours", totals.hours.toFixed(2), "Payroll hours", "gray")}
