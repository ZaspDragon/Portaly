...
const seedData = {
  ...
  timesheets: [
    { id: "t1", agencyId: "a1", workerId: "w1", clientId: "c1", weekStart: weekStart(), regularHours: 38.5, overtimeHours: 0, agencyApproved: true, clientApproved: true },
    { id: "t2", agencyId: "a1", workerId: "w2", clientId: "c1", weekStart: weekStart(), regularHours: 41.25, overtimeHours: 1.25, agencyApproved: true, clientApproved: false },
    { id: "t3", agencyId: "a1", workerId: "w3", clientId: "c2", weekStart: weekStart(), regularHours: 29.75, overtimeHours: 0, agencyApproved: false, clientApproved: false },
    { id: "t4", agencyId: "a2", workerId: "w4", clientId: "c3", weekStart: weekStart(), regularHours: 40, overtimeHours: 0, agencyApproved: true, clientApproved: true }
  ]
};
...
function normalize(data) {
  ...
  merged.timesheets = merged.timesheets.map(t => ({ ...t, clientId: t.clientId || assignmentForWorkerInData(merged, t.workerId)?.clientId || null }));
  return merged;
}
...
function timesheetsView() {
  ...
  return `
    <div class="card">
      ...
      ${rows.length ? `<div class="table-wrap"><table><thead><tr><th>Week</th><th>Worker</th><th>Client</th><th>Regular</th><th>OT</th><th>Total</th><th>Agency</th><th>Client</th><th>Action</th></tr></thead>
      <tbody>${rows.map(t => `<tr>
        <td>${t.weekStart}</td><td>${workerName(t.workerId)}</td><td>${clientName(t.clientId)}</td><td>${t.regularHours}</td><td>${t.overtimeHours}</td><td>${(Number(t.regularHours) + Number(t.overtimeHours)).toFixed(2)}</td>
        ...
        <td>${state.activeRole === "worker" ? '<span class="muted small">View only</span>' : `<button class="secondary-btn" data-approve="${t.id}">Approve</button>`}</td>
...
function timeclockView() {
  const workers = state.activeRole === "worker" ? [activeWorker()].filter(Boolean) : rowsFor("workers");
  const sites = state.activeRole === "client_manager" ? clientSites() : state.activeRole === "worker" ? rowsFor("sites") : agencySites();
  return `
    <div class="worker-clock card">
      <p class="eyebrow">${agency().agencyNumber}</p>
      <h3>${state.activeRole === "worker" ? "My Timeclock" : "QR Timeclock"}</h3>
      <div class="clock-face" id="clockFace">--:--:--</div>
      <div class="form-grid">
        <div>
          <label>Worker</label>
          <select id="clockWorker" ${state.activeRole === "worker" ? "disabled" : ""}>${workers.map(w => `<option value="${w.id}" ${w.id === state.activeWorkerId ? "selected" : ""}>${w.workerNumber} · ${w.firstName} ${w.lastName}</option>`).join("")}</select>
        </div>
        <div>
      ...
      <p class="muted small">Punches save worker, agency, job site, time, GPS status, and approval status.</p>
    </div>
    <div class="card" style="margin-top:14px;">
      <h3>Recent Punches</h3>
      ${punchTable(rowsFor("punches").slice(-10).reverse())}
    </div>
  `;
}
...
function createPunch(type) {
  const workerId = document.getElementById("clockWorker")?.value || state.activeWorkerId || agencyWorkers()[0]?.id;
